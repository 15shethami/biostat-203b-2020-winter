...
Before running a logistic regression, the ICD-9 Codes are recoded as an indicator variable with 1 representing the ICD-9 code for the patient is in the top five most common types of pneumonia. Additionally, all character variables are converted into factors. The `cohort_admissions` data is split into a training and test set with 75% of the observations in the training set. A logistic regression is performed on the training set with age, drug severity, ICD-9 code, principal drug, gender, and ethnic group as the covariates.
