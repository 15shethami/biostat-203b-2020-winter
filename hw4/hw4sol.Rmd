...
logit <- glm(mortality ~ drg_severity + principal_dx + gender + 
...
# Normalize the data
xtrain_norm <- as.data.frame(lapply(x_train, normalize))
xtrain_norm <- as.matrix(xtrain_norm)
dimnames(xtrain_norm) <- NULL

y_train <- as.matrix(y_train)
dimnames(y_train) <- NULL
to_categorical(y_train)

image(t(xtrain_norm[1, 8:1]), useRaster=TRUE, axes=FALSE, col=grey(seq(0, 1, length = 256)))
