...
logit <- glm(mortality ~ drg_severity + icd9_code + principal_dx + gender + 
...
rec_obj <- recipe(mortality ~ ., data = train2) %>%
...
x_train_tbl <- bake(rec_obj, newdata = train_tbl) %>% select(-mortality)
x_test_tbl  <- bake(rec_obj, newdata = test_tbl) %>% select(-mortality)

columns <- c("drg_severity", "icd9_code", "gender", "ethnic_group")
final_cohort2 <- final_cohort1
final_cohort2[,columns] <- lapply(final_cohort2[, columns], as.numeric)
final_cohort2$mortality <- ifelse(final_cohort2$mortality==TRUE, 1, 0)
final_cohort2$principal_dx <- ifelse(final_cohort2$principal_dx==TRUE, 1, 0)
...
#x_train[, 8:1]
#image(t(x_train[, 8:1]), useRaster=TRUE, axes=FALSE, col=grey(seq(0, 1, length = 256)))
