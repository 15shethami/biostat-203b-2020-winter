...
```{r}
admission %>% distinct(SUBJECT_ID, .keep_all = TRUE) %>% ggplot() + geom_bar(mapping = aes(x = LANGUAGE)) + ggtitle("Plot of Patient Language") + theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 90, hjust = 1, size = 6))

#scale_x_discrete(breaks = admission$LANGUAGE[!is.na(admission$LANGUAGE)])
```
...
```{r}
admission %>% distinct(SUBJECT_ID, .keep_all = TRUE) %>% ggplot() + geom_bar(mapping = aes(x = ETHNICITY)) + ggtitle("Plot of Patient Ethnicity") + theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 75, hjust = 1, size = 6)) + scale_x_discrete(drop = F)
```
...
First let's try to read and parse the `PATIENTS.csv` data.
...
```{r}
patient_adm <- inner_join(patient, admission, by = "SUBJECT_ID")
```
...
```{r}
patient_adm %>% ggplot() + geom_bar(mapping = aes(x = factor(GENDER), y = ..prop.., group = 1), stat = "count") + scale_x_discrete(label = c("Female", "Male")) + labs(title = "Gender of Patients", y = "Proportion", x = "Gender")
```

- Age at admission 

```{r}
patient_adm %>% mutate(age_adm = year(ADMITTIME) - year(DOB)) %>% ggplot() + geom_bar(mapping = aes(x = age_adm))
