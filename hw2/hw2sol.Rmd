...
```{r}
patient_adm %>% mutate(age_adm = as.numeric(as.duration(ADMITTIME - DOB), "years")) %>% ggplot() + geom_histogram(mapping = aes(x = age_adm), bins = 40)
```
...
icu %>% ggplot() + geom_histogram(mapping = aes(x = LOS), bins = 100) + labs(title = "Plot of Length of ICU Stay", x = "Length of Stay (days)")
...
```{r}
icu %>% distinct(SUBJECT_ID, .keep_all = TRUE) %>% ggplot() 
```
