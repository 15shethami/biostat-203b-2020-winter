...
```
<!-- difference between no F, -F:, and -F, -->
...
<!-- identify column with name instead of number? -->
```{bash}
...
**Solution:** Let's display the number of times each of the four characters (Elizabeth, Jane, Lydia, and Darcy) is mentioned respectively <!-- do not add to git will this be reproducible --> 
```{bash}
for name in Elizabeth Jane Lydia Darcy
      do
        echo $name
        grep $name pride_and_prejudice.txt | wc -l
      done
```
...
Rscript runSim.R seed=203 n=100 dist=gaussian rep=50
```
