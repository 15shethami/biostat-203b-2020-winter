...
<!-- UNDO LATER BUT TAKING A LONG TIME TO RUN -->
```{bash, eval=FALSE}
 for datafile in /home/203bdata/mimic-iii/*.csv
  do
   wc -l $datafile
...
**Solution:** Let's display the number of rows in this data file
```{bash}
...
**Solution:** Let's display the possible values taken by each of the variable `INSURANCE`, `LANGUAGE`, `RELIGION`, `MARITAL_STATUS`, and `ETHNICITY`
...
**Solution:** The first command directs output from one command to a file, so 'hello, world' is copied into a new script. The second command appends output from one command to a file, so 'hello, world' could be added to a new or previously existing script. Since 'hello, world' is appended to an empty script, the output from the first and second command appear to be the same.
...
**Solution:** 1 is the filename, tells us to run head on the pride and prejudice text, 2 is the argument that tells us how many lines to head, 3 tells us how many lines to display from the bottom of `heads` output; the first line tell us which shell to run the `middle.sh` file in 
...
Let's output the file from the simulations with combinations of sample sizes and distributions
```{bash}
...
```{r message=FALSE}
#library("tidyverse")
read_delim("n100_dist_gaussian.txt", delim = " ", col_names = F, col_types = cols())
read_csv("n100_dist_gaussian.txt", col_names = F, col_types = cols())
simulations
file_list <- list.files(path=".", pattern="*.txt") 
simulations<- file_list[1:15]
read_csv(simulations, col_names = F, col_types = cols())
datalist = lapply(simulations, function(x)(read_csv(x, col_names = F, col_types = cols())))
