...
col <- as.matrix(col)
col[is.na(col)] <- "Taiwan"
col

ncov_tbl %>%
  filter(`Country/Region` %in% c("China", "Taiwan*")) %>% 
  distinct(`Province/State`, `Country/Region`) %>%
  mutate_all(~replace(., is.na(.), "Taiwan")) %>% 
  print(n = Inf)
```
...
#"^HSI", Dow Jones (^DJI), NASDAQ (^IXIC), Russell 2000 (^RUT), 
#FTSE 100 (^FTSE), Nikkei 225 (^N225), HANG SENG INDEX (^HSI)
 ...
 stock <- function(symb){
  ...
  annotate(geom ="rect", x =3, y = 24, label="Start of Data on Coronavirus",
              color="red")
}
